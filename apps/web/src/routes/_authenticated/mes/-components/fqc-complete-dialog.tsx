...
const formSchema = z
	...
	}) satisfies z.ZodType<Parameters<ReturnType<typeof client.api.fqc>["complete"]["post"]>[0]>;

export type FqcCompleteFormValues = z.infer<typeof formSchema>;

const buildDefaultValues = (): FqcCompleteFormValues => ({
	decision: "PASS",
	failedQty: 1,
	remark: "",
});

interface FqcCompleteDialogProps {
	open: boolean;
	onOpenChange: (open: boolean) => void;
	fqc: FqcDetail | null;
	onSubmit: (values: FqcCompleteFormValues) => Promise<void>;
	isSubmitting?: boolean;
}

export function FqcCompleteDialog({
	open,
	onOpenChange,
	fqc,
	onSubmit,
	isSubmitting,
}: FqcCompleteDialogProps) {
	const form = useForm({
	...
	const sampleQty = fqc?.sampleQty ?? undefined;
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-[520px]">
				<DialogHeader>
					<DialogTitle>完成末件检验</DialogTitle>
					<DialogDescription>提交最终检验结论。</DialogDescription>
				</DialogHeader>
				<form
					onSubmit={(event) => {
						...
						>
							{(field) => (
								<Select
									...
									onValueChange={(value) =>
										field.handleChange(value as FqcCompleteFormValues["decision"])
									}
								>
