...
}) satisfies z.ZodType<Parameters<ReturnType<typeof client.api.fqc>["items"]["post"]>[0]>;

export type FqcRecordFormValues = z.infer<typeof formSchema>;

const buildDefaultValues = (): FqcRecordFormValues => ({
	unitSn: "",
	itemName: "",
	itemSpec: "",
	actualValue: "",
...
interface FqcRecordDialogProps {
	open: boolean;
	onOpenChange: (open: boolean) => void;
	fqc: FqcDetail | null;
	onSubmit: (values: FqcRecordFormValues) => Promise<void>;
	isSubmitting?: boolean;
	readOnly?: boolean;
}

export function FqcRecordDialog({
	open,
	onOpenChange,
	fqc,
	onSubmit,
	...
	const canRecord = !readOnly && fqc?.status === "INSPECTING";
	const statusLabel = fqc ? (INSPECTION_STATUS_MAP[fqc.status] ?? fqc.status) : "-";

	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="sm:max-w-[720px]">
				<DialogHeader>
					<DialogTitle>末件检验记录</DialogTitle>
					<DialogDescription>
						{fqc?.run?.runNo ? `批次 ${fqc.run.runNo} 的末件检验记录。` : "检验记录详情"}
					</DialogDescription>
				</DialogHeader>
				<form
					onSubmit={(event) => {
					...
					<DialogBody className="space-y-6">
						<div className="grid gap-4 md:grid-cols-4">
							<div>
								...
								<Badge variant={fqc?.status === "FAIL" ? "destructive" : "secondary"}>
									{statusLabel}
								</Badge>
							</div>
							<div>
								<p className="text-sm text-muted-foreground">抽样数量</p>
								<p className="font-medium">{fqc?.sampleQty ?? "-"}</p>
							</div>
							<div>
								<p className="text-sm text-muted-foreground">通过数量</p>
								<p className="font-medium">{fqc?.passedQty ?? 0}</p>
							</div>
							<div>
								<p className="text-sm text-muted-foreground">失败数量</p>
								<p className="font-medium">{fqc?.failedQty ?? 0}</p>
							</div>
						</div>
						...
						<div className="space-y-2">
							<div className="flex items-center justify-between">
								...
								<span className="text-sm text-muted-foreground">
									共 {fqc?.items?.length ?? 0} 项
								</span>
							</div>
							{fqc && fqc.items.length > 0 ? (
						...
						{canRecord && (
							<div className="space-y-4">
								...
								<div className="grid gap-4 md:grid-cols-2">
									...
									<Field form={form} name="result" label="结果">
										{(field) => (
											<Select
												value={field.state.value}
												onValueChange={(value) =>
													field.handleChange(value as FqcRecordFormValues["result"])
												}
											>
												<SelectTrigger>
													<SelectValue />
									...
									<Field form={form} name="defectCode" label="缺陷代码（可选）">
										{(field) => (
											<Input
												value={field.state.value}
												onBlur={field.handleBlur}
												onChange={(event) => field.handleChange(event.target.value)}
												className="w-full"
												placeholder={result === "FAIL" ? "例如: D001" : "可选"}
											/>
										)}
									</Field>

									<div className="md:col-span-2">
										<Field form={form} name="itemSpec" label="标准（可选）">
											{(field) => (
												<Input
													value={field.state.value}
													onBlur={field.handleBlur}
									...
									<div className="md:col-span-2">
										<Field form={form} name="actualValue" label="实测值（可选）">
											{(field) => (
												<Input
													value={field.state.value}
													onBlur={field.handleBlur}
