...
function RouteListPage() {
	...
	const handleView = (routingCode: string) => {
		navigate({ to: "/mes/routes/$routingCode", params: { routingCode } });
	};
		...
		<DataListLayout
			...
			filterToolbarProps={{
				...
				onFilterChange: (key, value) => {
					navigate({
						to: ".",
						search: { ...searchParams, [key]: value || undefined, page: 1 },
						replace: true,
					});
				},
				onReset: () =>
					navigate({ to: ".", search: { page: 1, pageSize: searchParams.pageSize }, replace: true }),
