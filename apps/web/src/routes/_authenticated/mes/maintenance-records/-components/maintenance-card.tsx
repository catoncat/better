...
export function MaintenanceCard({ record }: MaintenanceCardProps) {
	const primaryField = maintenanceFieldMeta.find((field) => field.cardPrimary);
	const secondaryField = maintenanceFieldMeta.find((field) => field.cardSecondary);
	const badgeField = maintenanceFieldMeta.find((field) => field.cardBadge);
	const detailFields = maintenanceFieldMeta.filter((field) => field.cardDetail);
