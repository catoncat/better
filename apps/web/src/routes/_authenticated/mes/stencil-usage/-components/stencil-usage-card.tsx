...
interface StencilUsageCardProps {
	record: StencilUsageRecord;
}

export function StencilUsageCard({ record }: StencilUsageCardProps) {
	const primaryField = stencilUsageFieldMeta.find((field) => field.cardPrimary);
	const secondaryField = stencilUsageFieldMeta.find((field) => field.cardSecondary);
	const badgeField = stencilUsageFieldMeta.find((field) => field.cardBadge);
	const detailFields = stencilUsageFieldMeta.filter((field) => field.cardDetail);
