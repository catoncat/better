...
import type { StationType } from "@better-app/db";
...
type RoutingServiceModule = typeof import("../modules/mes/routing/service");
...
const seedMesMasterData = async (prisma: Db["default"], db: Db) => {
	...
	const opRecords: Record<string, { id: string; defaultType: StationType }> = {};
	...
	for (const step of smtSteps) {
		const opRecord = opRecords[step.opCode];
		if (!opRecord) {
			throw new Error(`Missing operation seed: ${step.opCode}`);
		}
		await prisma.routingStep.create({
			data: {
				...
				stationType: opRecord.defaultType,
...
export const runSeed = async (options: SeedOptions = {}) => {
	...
	const routingService = (await import("../modules/mes/routing/service")) as RoutingServiceModule;
