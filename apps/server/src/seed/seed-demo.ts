...
export const runSeedDemo = async ({ prisma, now }: SeedDemoOptions = {}) => {
		...
		if (unitData.status === db.UnitStatus.DONE || unitData.currentStepNo > 1) {
			for (let stepNo = 1; stepNo <= unitData.currentStepNo; stepNo++) {
				...
				if (!existingTrack) {
					...
					const outAt =
						stepNo < unitData.currentStepNo || unitData.status === db.UnitStatus.DONE
	...
	console.log(
		"  ✓ WO-MGMT-SMT-EXEC (IN_PROGRESS) → RUN-MGMT-SMT-EXEC (IN_PROGRESS) + 5 units with tracks",
	);
	...
	// 3. 统计
	console.log("\n========== Demo Data Summary ==========");
	const woCount = await prismaClient.workOrder.count({
		where: { woNo: { startsWith: "WO-MGMT" } },
	});
