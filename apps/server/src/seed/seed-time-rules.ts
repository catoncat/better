import prisma, { type PrismaClient, TimeRuleScope, TimeRuleType } from "@better-app/db";
 ...
 */
export async function seedTimeRules(prismaClient: PrismaClient = prisma) {
	console.log("Seeding time rule definitions...");

	for (const rule of PRESET_TIME_RULES) {
		await prismaClient.timeRuleDefinition.upsert({
