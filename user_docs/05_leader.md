...
#### 1.5 收尾确认（关闭批次）
批次进入终态后进行收尾确认：
1. 确认批次已进入终态（`COMPLETED` / `CLOSED_REWORK` / `SCRAPPED`）
2. 点击 **关闭批次**（收尾确认）
3. 确认提交

> **说明**：收尾确认不改变批次终态，用于完成收尾与归档确认。
...
开始生产 │
    ...
    [IN_PROGRESS] 生产中
        │
        ├── OQC通过/无OQC → [COMPLETED] 已完成
        └── OQC不合格 → [ON_HOLD] 批次隔离
                          │
                          └── MRB决策 → [COMPLETED]/[CLOSED_REWORK]/[SCRAPPED]
```

> **终态说明**：`CLOSED_REWORK` 表示批次因返修闭环，`SCRAPPED` 表示整批报废。
...
### 场景4: 批次完成
1. 确认 OQC/MRB 处理结果，批次进入终态（`COMPLETED` / `CLOSED_REWORK` / `SCRAPPED`）
2. 处理异常产品（如有）
3. 执行收尾确认（关闭批次）
4. 准备下一批次
