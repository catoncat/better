...
**问题描述**:
- 当前有 4 种作用域：ROUTE / OPERATION / STEP / SOURCE_STEP
- STEP 和 SOURCE_STEP 在实现中被合并处理，但 UI 分开展示
- OPERATION 层（跨路由共享配置）需求不常见，增加理解成本
- 用户难以理解什么时候用哪种作用域

**代码实现分析** (`routing/service.ts:439-451`):
```typescript
// STEP 和 SOURCE_STEP 被合并
const stepConfig = pickLatest([...stepConfigs, ...sourceConfigs]);

// 然后按优先级覆盖
const stationGroupId =
    stepConfig?.stationGroupId ??       // STEP/SOURCE_STEP
    operationConfig?.stationGroupId ??  // OPERATION
    routeConfig?.stationGroupId ??      // ROUTE
    step.stationGroupId;                // 默认值
```
