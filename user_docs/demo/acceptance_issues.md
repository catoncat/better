...
| 6 | 5.3 | Major | 前端错误提示不准确（403 权限错误显示为"检查网络"） | Open |
...
## 问题 #6: 前端错误提示不准确（403 权限错误）
- **阶段**: 5.3（API 错误处理）
- **页面**: `/mes/runs/:runNo`（批次详情页）
- **严重程度**: Major（用户无法理解真正的错误原因）
- **描述**:
  - 现象：使用 `planner` 账号点击就绪检查时，后端返回 403 `Missing required permission: readiness:check`，但前端显示"预检失败 检查网络"。
  - 原因：前端错误处理未区分 403 权限错误，统一显示网络错误提示。
  - 预期：应显示"权限不足：缺少 readiness:check 权限"或类似提示。
- **复现步骤**:
  1. 登录 `planner@example.com`
  2. 打开批次详情页
  3. 点击「执行预检」
  4. 观察：前端显示"预检失败 检查网络"，后端日志显示 403 权限错误
- **状态**: Open
