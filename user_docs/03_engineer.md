...
- ERP 路由通过接口同步
- MES 原生路由通过数据导入或种子数据
如需新增路由，请通过 ERP 或数据导入流程完成。

#### 1.3 路由详情与执行语义配置
进入路由详情页可查看步骤列表（只读，ERP 来源步骤不可编辑），并维护执行语义配置：

- 工艺类型：在路由信息中更新 processType
- 作用域：整条路由 / 单步骤
- 站点约束：stationType、站点组、允许站点
- 门禁：requiresFAI / requiresAuthorization
- 数据采集：绑定 DataSpec
- ingestMapping / meta（JSON）

可使用“快速为缺失步骤配置站点组”补齐约束。
...
2. 点击 **编译** 按钮
3. 编译成功后生成新版本
4. 若执行语义缺失，版本将标记为 INVALID 并提示错误

> **说明**: 编译后的版本不可修改，如需变更需重新编译生成新版本。
