...
| **合计** | **17** | **9 (53%)** | **4 (24%)** | **4 (23%)** |

## 6. 待完善项

### 高优先级
- [ ] 场景 4：追溯查询 API 验证（`GET /api/trace/units/{sn}`）
- [ ] 场景 6：并发测试脚本

### 中优先级
- [ ] 场景 10：连续失败触发锁定
- [ ] 场景 16：手动降级模式（`source=MANUAL`）

### 低优先级
- [ ] 场景 8：大批量数据处理（性能测试）
- [ ] 场景 15：锡膏过期阻断（扩展 `readiness-waive`）
