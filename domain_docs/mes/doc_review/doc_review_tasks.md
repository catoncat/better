...
> 状态：已确认
> 更新时间：2026-01-28
> 目标：建立“文档为真源”的系统性 Review 机制（Meta 拆解 + 模板 + 轮次规则），作为后续细拆与执行的输入。

---

## 0. 角色与定位

- 本文档只定义 **Meta 任务拆解**（规则、模板、轮次策略与产出结构）。
...
- `roundX_*.md`：轮次对齐矩阵与偏差清单（执行阶段产出）
- `99_high_risk_findings.md`：高风险误导点汇总（执行阶段产出）
- `00_status.md`：共享进度状态（轮次/高风险偏差追踪）

---

## 4.1 首次运行默认初始化（示例确认清单）

当用户首次要求“生成 review 任务 + 维护进度”时，默认做**可用初始化**，并先给出明确的“拟更新内容清单”：

```
拟更新内容：
1) domain_docs/mes/doc_review/00_review_method.md
   - 生成方法论模板（真源层级 / 驱动入口 / 最小单元 / 产出）
2) domain_docs/mes/doc_review/00_alignment_matrix_template.md
   - 生成对齐矩阵模板（文档↔API↔UI）
3) domain_docs/mes/doc_review/00_review_backlog.md
   - 生成功能点清单模板（API 域 / 文档 / UI / 轮次）
4) domain_docs/mes/doc_review/doc_review_tasks.md
   - 状态：草案（待确认）→ 已确认
5) domain_docs/mes/doc_review/00_status.md
   - 轮次：轮次1-3 保持 TBD（不定义范围）

请确认是否按上述内容更新（是/否）
```

> 说明：首次初始化产出“可用模板”，避免只有状态变更但无实际产出。
> 默认会补齐 Backlog 初始条目（按 API 域），不进入“改模板/结构”路径，除非用户明确要求。
...
- T-DR-1.2 建立轮次拆分规则与优先级矩阵（不锁死轮次顺序）

### T-DR-2 轮次执行（由细拆承接）
- T-DR-2.x 轮次对齐矩阵与偏差清单（每轮次一个文档）
- T-DR-2.y 高风险误导点汇总
