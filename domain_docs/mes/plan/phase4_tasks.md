...
- [ ] 4.6.1 前置确认：表单字段对齐 + 时间窗口起止事件 + 数据来源确认
  - DoD：`smt_forms/*.md` 字段版本确认；`QR-Pro-012` 字段补齐；“回流焊/AOI→水洗 <4h（按路由/工序配置）”“锡膏暴露 <24h”起止事件与违规策略明确（提醒 + 可豁免）

- [ ] 4.6.2 时间窗口规则（WP-1）
  - DoD：时间窗口规则可配置；超时提醒 + 可豁免（软门禁，不强制阻断）；Trace 可回溯规则命中

- [x] 4.6.3 烘烤记录（WP-2）
  - DoD：BakeRecord 数据结构 + UI 表单；可关联 Run 与物料批次

- [ ] 4.6.3.1 烘烤记录联动与规则（WP-2 后续）
  - DoD：烘烤记录接入准备检查或规则服务；异常/告警记录可追溯

- [x] 4.6.4 锡膏生命周期（WP-3）
  - DoD：锡膏解冻/回温/搅拌/领用/回收可追溯；冷藏温度记录可落地

- [ ] 4.6.4.1 锡膏记录联动（WP-3 后续）
  - DoD：锡膏使用记录接入时间窗口与准备检查；冷藏温度缺测/超限可追溯

- [x] 4.6.5 钢网/刮刀寿命（WP-4）
  - DoD：钢网/刮刀使用与点检记录落地，责任人与时间可追溯

- [ ] 4.6.5.1 钢网/刮刀寿命联动（WP-4 后续）
  - DoD：使用次数累计规则与寿命阈值预警落地；准备检查/Trace 可追溯

- [ ] 4.6.6 转拉前检查模板化（WP-5）
  - DoD：ReadinessCheck 支持模板化检查项与单签确认

- [ ] 4.6.7 FAI 签字与末件（WP-6）
  - DoD：签字人可追溯（单签）；末件检查类型可配置

- [x] 4.6.8 设备点检（WP-7）
  - DoD：AOI/SPI 设备每日点检表单落地（人工或导入），记录结果与责任人

- [ ] 4.6.8.1 设备点检异常联动（WP-7 后续）
  - DoD：点检失败触发异常反馈链路，支持追溯与处置记录

- [x] 4.6.9 炉温程式记录（WP-8）
  - DoD：炉温程式使用记录可落地（集成或人工）

- [x] 4.6.10 换料记录增强（WP-9）
  - DoD：包装数量、审核人字段落地；换料历史可查询

- [x] 4.6.11 日常 QC 与异常（WP-10）
  - DoD：日常 QC 与生产异常表单落地，记录停机与措施等关键字段

- [ ] 4.6.11.1 日常 QC 统计与异常闭环（WP-10 后续）
  - DoD：班次/时间段统计可生成；异常闭环（纠正措施/复核）可追溯

- [ ] 4.6.12 其它 SMT 表单确认与拆分
  - DoD：`smt_form_collection_matrix.md` 完成确认后拆分实施任务（如 QR-Mac-155、QR-Pro-121、QR-Pro-012、QR-Mac-134、QR-Tra-xxxx、生产数据记录表、产品出/入数记录表、QR-IQC-01、QR-Pro-073）
