...
| 角色 | 权限列表（摘要） | 数据范围 |
| --- | --- | --- |
| admin | SYSTEM_USER_MANAGE, SYSTEM_ROLE_MANAGE, SYSTEM_CONFIG, SYSTEM_INTEGRATION, WO_READ, RUN_READ, ROUTE_READ, TRACE_READ, EXEC_READ, READINESS_VIEW, LOADING_VIEW | ALL |
| planner | WO_READ, WO_RECEIVE, WO_RELEASE, WO_UPDATE, WO_CLOSE, RUN_READ, RUN_CREATE, RUN_AUTHORIZE, RUN_REVOKE, RUN_CLOSE, ROUTE_READ, TRACE_READ, READINESS_VIEW | ALL |
| engineer | SYSTEM_INTEGRATION, ROUTE_READ, ROUTE_CONFIGURE, ROUTE_COMPILE, ROUTE_CREATE, DATA_SPEC_CONFIG, OPERATION_CONFIG, LINE_CONFIG, WO_READ, RUN_READ, TRACE_READ, READINESS_CONFIG, LOADING_CONFIG | ALL |
...
| material | RUN_READ, LOADING_VIEW, LOADING_VERIFY | ASSIGNED_LINES |
| operator | EXEC_TRACK_IN, EXEC_TRACK_OUT, EXEC_DATA_COLLECT, TRACE_READ, READINESS_VIEW, LOADING_VIEW, LOADING_VERIFY | ASSIGNED_STATIONS |
| trace | TRACE_READ, TRACE_EXPORT | ALL |
...
| 页面 | admin | planner | engineer | quality | material | operator | trace |
| --- | --- | --- | --- | --- | --- | --- | --- |
| /mes/work-orders | YES | YES | YES | YES | NO | NO | NO |
| /mes/runs | YES | YES | YES | YES | YES | NO | NO |
| /mes/runs/:runNo | PARTIAL | PARTIAL | PARTIAL | YES | PARTIAL | NO | NO |
| /mes/execution | NO | NO | NO | NO | NO | YES | NO |
| /mes/fai | NO | NO | NO | YES | NO | NO | NO |
| /mes/oqc | NO | NO | NO | YES | NO | NO | NO |
| /mes/trace | YES | YES | YES | YES | NO | YES | YES |
| /mes/loading | YES | NO | NO | NO | YES | NO | NO |
| /mes/routes | YES | YES | YES | NO | NO | NO | NO |

图例：YES 完全可用；PARTIAL 部分功能可用；NO 无权限访问。
...
- 状态：OK 已保留无权限占位；MRB 按钮已有 gating  
...
**Dialog 审计**：  
- 豁免检查项：`readiness:override` OK  
- 创建 FAI：`quality:fai` OK  
- MRB 决策：`quality:disposition` OK  
- 批次收尾：`run:close` OK  
- 生成单件：`run:authorize` OK
...
- 状态：OK 按 `route:read` gating 并隐藏筛选  
...
- 状态：OK `useReadinessExceptions` 按 `readiness:view` gating  

3) 批次号跳转  
- Action 权限：`run:read`（进入批次详情）  
- 展示策略：缺 `run:read` 时降级为纯文本  
- 状态：OK 缺 `run:read` 时降级为文本  
...
- View 权限：`loading:view`  
 ...
 - 状态：OK 按 `loading:view` / `loading:verify` gating  

4) 上料历史记录  
- View 权限：`loading:view`  
- 展示策略：缺 view → 隐藏  
- 状态：OK `useLoadingRecords` 按 `loading:view` gating  

5) 站位解锁按钮  
- Action 权限：`loading:config`  
- 展示策略：缺权限 → 隐藏解锁入口  
- 状态：OK 按 `loading:config` 隐藏入口  
...
- 状态：OK 页面 view gating + 查询 enabled  
...
- 展示策略：缺权限 → 页面级无权限占位  
- 状态：OK 页面级 gating + `/stations` enabled  

2) 待执行批次列表（快捷选择）  
- View 权限：`run:read`  
- 展示策略：缺权限 → 隐藏  
- 状态：OK 缺权限隐藏 + 查询 gated  

3) 当前队列  
- View 权限：`exec:read` 或 `exec:track_in` / `exec:track_out`  
- Action 权限：`exec:track_out`（出站/报不良）  
- 展示策略：缺 view → 无权限占位；缺 action → 禁用按钮  
- 状态：OK 队列查询 gated + 按钮按权限禁用  
...
- 状态：OK 页面级 gating + list/detail enabled  

2) 批次筛选（Run Combobox）  
- View 权限：`run:read`  
- 展示策略：缺权限 → 隐藏筛选  
- 状态：OK 缺权限隐藏 + run list gated  
...
- 状态：OK 模板选择按权限 gating + query enabled  

5) 生成单件（FAI 开始时触发）  
- Action 权限：`run:authorize`  
- 展示策略：缺权限 → 不提供生成路径或提示无权限  
- 状态：OK 生成单件按 `run:authorize` gating  
...
- View 权限：`quality:oqc`  
- 展示策略：缺 view → 页面级无权限占位  
- 状态：OK 页面级 gating + list enabled  

2) 操作按钮（开始/记录/完成/查看）  
- Action 权限：`quality:oqc`  
- 展示策略：缺权限 → 隐藏或只读  
- 状态：OK 表格/卡片动作按权限 gating  

3) 记录/完成弹窗  
- Action 权限：`quality:oqc`  
- 展示策略：缺权限 → 不可打开  
- 状态：OK 入口 action gating + detail 查询 gated  
...
- View 权限：`route:read`  
- 展示策略：配置与运维模块，缺 view → 页面级无权限占位  
- 状态：OK 页面级无权限占位 + `useRouteList` enabled  

2) 详情入口（卡片/表格）  
- View 权限：`route:read`  
- 展示策略：缺权限 → 隐藏入口  
- 状态：OK 页面级 gating（无权限不渲染列表）  
...
- View 权限：`route:read`  
- 展示策略：配置与运维模块，缺 view → 页面级无权限占位  
- 状态：OK 页面级 gating（`useRouteDetail` enabled）  

2) 执行语义配置列表  
- View 权限：`route:read`  
- 展示策略：缺 view → 页面级无权限占位  
- 状态：OK `useExecutionConfigs` enabled  
...
- View 权限：`route:read`  
- 展示策略：配置与运维模块，缺 view → 页面级无权限占位  
- 状态：OK `RouteSelect`/`useRouteVersions` 按 route:read gating  
...
- View 权限：`data_spec:read` + `data_spec:config`  
- 展示策略：配置与运维模块，缺 view → 页面级无权限占位  
- 状态：OK 页面级无权限占位 + list enabled  

2) 工序筛选  
- View 权限：`operation:read` + `data_spec:config`  
- 展示策略：缺权限 → 隐藏筛选  
- 状态：OK `useOperationList` enabled  

3) 新建/编辑/启停  
- Action 权限：`data_spec:config`  
- 展示策略：缺权限 → 隐藏  
- 状态：OK 表格/卡片动作已 gating  
...
- 展示策略：缺权限 → 隐藏提交按钮  
- 状态：OK 页面级 system:integration gating + `<Can>` 提交控制  

2) 产线绑定（选择产线 + 绑定按钮）  
- View 权限：`run:read` + `run:create`  
- Action 权限：`loading:config`  
- 展示策略：缺 view → 隐藏产线选择；缺 action → 隐藏绑定按钮  
- 状态：OK 产线列表 query gating + 无权限提示 + `<Can>` 按钮  
