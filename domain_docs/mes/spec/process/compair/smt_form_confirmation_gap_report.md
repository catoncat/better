...
| 准备完成口径 | 所有准备项达标；门禁=No 允许继续但提醒/豁免并记录 | Readiness 仅覆盖部分项；无软门禁提醒/豁免机制 | 需补齐准备项清单与软门禁规则 |
| 时间规则（回流焊→水洗 4h） | 仅提醒（Alert），需可触发豁免 | 未见水洗工序/规则、无时间规则检查 | 规则未实现，缺少提醒与豁免机制 |
| 时间规则（锡膏暴露 <24h） | 仅提醒（Alert），可豁免 | 仅有锡膏状态/使用记录；无暴露时长规则 | 规则未实现 |
| 豁免权限 | 厂长 (Factory Manager) | 预设角色无厂长角色；仅有 Readiness Override 权限 | 角色/权限缺口，需新角色或映射策略 |
...
| QR-Pro-133 转拉前检查项目 | 系统运算结果 | Readiness 检查覆盖部分项 | 覆盖不完整（程式/夹具/辅料缺失） |
