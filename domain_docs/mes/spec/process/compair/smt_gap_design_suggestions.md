...
- **门禁类型分层**  
  ...
  - **仅记录（RECORD）**：不达标不阻塞，但**仍需强制记录**（不可跳过）
...
建议：
- **复用现有 `admin` 角色**：授予 `time_rule:override` 权限（避免新增角色）
- 若业务需求明确区分"厂长"，则在 `admin` 下新增 `factory_manager` 子角色
...
| 问题 | 选项 | 建议 |
|------|------|------|
| 非门禁项是否强制确认？ | A) 仅记录不强制 B) 必须确认才能继续 | **B**（已明确） |
| 豁免权限映射哪个角色？ | A) 新增 factory_manager B) 复用 admin | **B 优先**，避免角色膨胀 |
| 水洗时间规则适用范围？ | A) 全局 B) 路由/产品配置 | **B**（非所有产品需水洗） |
| 准备项用独立表还是扩展 Readiness？ | A) 新表 B) 扩展 ReadinessCheckType | **B**（单一数据源） |
