...
按用户话术自动判断意图：
- **任务/清单/计划/拆解/模板** → 生成/更新 review 任务清单（改 `doc_review_tasks.md` 或模板）
- **进度/状态/推进/完成/卡住** → 维护共享进度（改 `00_status.md`）
- **规则/方法/流程** → 更新规则/方法（改 `doc_review_tasks.md` 或方法论说明）
...
**默认模式（首次运行或未给具体目标）**：
- 当用户仅输入 `$doc-review-meta` 或“做文档 review 任务+进度”但没有具体改动点，直接进入“初始化基线”流程。
- 仅给出一份“拟更新内容清单”，然后问“是否执行（是/否）”。不提供 A/B/C 选项。
