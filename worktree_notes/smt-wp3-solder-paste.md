...
- [x] Slice 1: DB schema + migration (solder paste usage + cold storage temp)
- [x] Slice 2: Server API + audit entries + routes wiring
- [x] Slice 3: Web UI list + create dialog + hooks
- [x] Slice 4: Docs align + plan checkbox update
...
- Field meta pattern defines card/table rendering in one place; columns derived via `createColumnsFromFieldMeta`.
- FilterToolbar uses `type: "dateRange"` with `dateFromKey`/`dateToKey` and passes `filters` + `onFilterChange`/`onFiltersChange`.
- Card component selects primary/secondary/badge/detail fields from field meta; dialog uses `DateTimePicker` and TanStack Form with normalization on submit.
- `useLines` hook provides line list via `client.api.lines.get` for line selection inputs.
- `apps/web/src/routeTree.gen.ts` is auto-generated by TanStack Router; avoid manual edits and regenerate via tooling.
- `tanstackRouter` Vite plugin is configured; route tree regeneration should happen via web build/dev.
- SMT align file currently only lists solder paste compliance checks; WP-3 needs new rows for usage and cold storage temp records.
- New API uses `lineCode` (not lineId) for solder paste usage creation to match LineSelect behavior.
- Phase4 plan still has 4.6.4 unchecked; needs update once WP-3 is fully landed.
