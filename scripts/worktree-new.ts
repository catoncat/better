...
function usage(): never {
	die(
		[
			...
			"- If .task-context.json exists, automatically merges its content (and deletes it after).",
			"",
			"Options:",
			...
			"  --no-context            Do not read .task-context.json",
		].join("\n"),
	);
}
...
function loadTaskContext(repoRoot: string): TaskContext | null {
	...
	try {
		...
		console.warn(`Warning: Failed to parse ${contextPath}, ignoring.`);
		return null;
	}
}

function deleteTaskContext(repoRoot: string): void {
	const contextPath = path.join(repoRoot, ".task-context.json");
	if (existsSync(contextPath)) {
		try {
			unlinkSync(contextPath);
			console.log(`Consumed and deleted: .task-context.json`);
		} catch {
			console.warn(`Warning: Failed to delete ${contextPath}`);
		}
	}
}

function mergeWithTaskContext(
	parsed: ParsedArgs,
	...
	return {
		...parsed,
		task: parsed.task ?? context.task,
		planPath: parsed.planPath ?? context.planPath,
		planItem: parsed.planItem ?? context.planItem,
		triageNotePath: parsed.triageNotePath ?? context.triageNotePath,
		touchPoints:
			parsed.touchPoints.length > 0
				? parsed.touchPoints
				: (context.touchPoints ?? []),
		slices: parsed.slices.length > 0 ? parsed.slices : (context.slices ?? []),
	};
}
...
function resolveBaseRef(
	...
	for (const candidate of candidates) {
		const res = run(["git", "rev-parse", "--verify", "--quiet", candidate], {
			cwd: worktreePath,
		});
...
function ensureAutoBlock(text: string): string {
	if (
		text.includes("<!-- AUTO:BEGIN status -->") &&
		text.includes("<!-- AUTO:END status -->")
	)
		return text;
...
function buildNoteTemplate(params: {
	...
	if (
		...
		if (params.triageNotePath)
			yaml.push(`  triageNote: ${yamlString(params.triageNotePath)}`);
	}
	...
	const title = params.task
		? `${params.branch} - ${params.task}`
		: params.branch;
	const sliceLines =
...
function updateNoteStatus(params: {
	...
	const noteRel = path
		.relative(params.worktreePath, params.notePath)
		.replaceAll(path.sep, "/");
	const shouldCommitNote =
		changedFiles.includes(noteRel) ||
		changedFiles.some((f) => f.startsWith("worktree_notes/"));
	...
	if (changedFiles.length > 50)
		statusLines.push(`  - ... (+${changedFiles.length - 50} more)`);
	statusLines.push("- Next:");
	if (shouldCommitNote) {
		statusLines.push(
			`  - Commit worktree note: git add ${noteRel} && git commit -m "docs(worktree): add task context"`,
		);
	} else {
		statusLines.push("  - Continue the next unchecked slice.");
	}

	const original = existsSync(params.notePath)
		? readFileSync(params.notePath, "utf8")
		: "";
...
// Main execution
const rawParsed = parseArgs(process.argv);
const branch = rawParsed.branch;
const targetPathArg = rawParsed.targetPathArg;

const repoRootResult = run(["git", "rev-parse", "--show-toplevel"]);
if (repoRootResult.exitCode !== 0)
	die(repoRootResult.stderr || "git rev-parse failed");
const mainRoot = repoRootResult.stdout.trim();
if (!mainRoot) die("Unable to determine git toplevel");

// Load and merge task context if available
const taskContext = rawParsed.noContext ? null : loadTaskContext(mainRoot);
const parsed = mergeWithTaskContext(rawParsed, taskContext);

if (taskContext) {
	console.log("Found .task-context.json, merging into worktree note...");
	if (taskContext.task) console.log(`  - task: ${taskContext.task}`);
	if (taskContext.slices?.length)
		console.log(`  - slices: ${taskContext.slices.length}`);
	console.log("");
}
...
if (addResult.exitCode !== 0)
	die(addResult.stderr || addResult.stdout || "git worktree add failed");
...
if (!parsed.noNote) {
	...
	if (note.created)
		noteNext = `- git add ${note.noteRel} && git commit -m "docs(worktree): add task context"`;
}

// Delete task context file after successful worktree creation
if (taskContext) {
	deleteTaskContext(mainRoot);
}
